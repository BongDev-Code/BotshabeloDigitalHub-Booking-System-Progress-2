...
async function handleLogin(e) {
  ...
  try {
    ...
    if (email === "receptionist@botshabelo.com" && password === "admin123") {
      currentUser = {
        id: "1",
        name: "Receptionist",
  ...
  bookings.forEach((booking) => {
    ...
    if (daysUntil > 28) {
      ...
      const weeksSinceBooking = Math.floor(
        (today - new Date(booking.created || booking.date)) / (1000 * 60 * 60 * 24 * 7),
      )
      ...
      <div class="reminder-card">
        <div class="reminder-header">
          <div class="reminder-title-row">
            ...
            <span class="urgency-badge urgency-${urgency.class}">${urgency.label}</span>
          </div>
        ...
        <div class="reminder-content">
          ...
          <div class="reminder-actions">
            <button class="btn-mark-sent" onclick="markReminderSent('${reminder.id}')">
              Mark as Notified
            </button>
          </div>
        </div>
...
function createRecurringReminders(booking) {
  ...
  if (daysUntil > 28) {
    ...
    while (reminderDate < bookingDate) {
      const daysUntilFromReminder = Math.ceil((bookingDate - reminderDate) / (1000 * 60 * 60 * 24))
...
function renderCalendar(filter = {}) {
  ...
  if (filter.search) {
    ...
    bookings = bookings.filter((b) => b.event.toLowerCase().includes(s) || b.name.toLowerCase().includes(s))
  }
  // Filter by date
  if (filter.date) {
  ...
  bookings.forEach((booking, idx) => {
    ...
    card.innerHTML = `
            ...
            People: ${booking.people || 1}, Duration: ${booking.duration || 1} hour(s)<br>
            Reminder: ${booking.reminder === "1hour" ? "1 hour" : "1 day"} before<br>
      ...
      setTimeout(() => {
        if (Notification.permission === "granted") {
          new Notification("Event Reminder", {
            body: `Reminder: "${booking.event}" is coming up at ${formatTime(booking.time)}!`,
            icon: "",
          })
        } else {
          showToast(`Reminder: "${booking.event}" is coming up at ${formatTime(booking.time)}!`)
        }
      }, timeout)
    }
  })
...
function deleteBooking(idx) {
  ...
  if (editIndex == idx) {
    ...
    document.querySelector("#bookingForm button[type='submit']").textContent = "Book Now"
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (bookingForm) {
    bookingForm.addEventListener("submit", function (e) {
      ...
      const people = Number.parseInt(document.getElementById("people").value, 10) || 1
      const duration = Number.parseInt(document.getElementById("duration").value, 10) || 1
...
function updateDurationLabel() {
  ...
  const perPersonFacilities = ["Board Room", "Meeting Room", "PC Labs", "Makerspace"]
...
document.getElementById("facility").addEventListener("change", updateDurationLabel)
...
function updateCostCalculator(syncToBooking = true) {
  ...
  const cateringPeople = catering ? Number.parseInt(document.getElementById("cateringPeople").value, 10) || 1 : 0
  ...
  if (category && facility && duration > 0) {
    ...
    if (typeof price !== "undefined") {
      ...
      if (perPersonFacilities.includes(facility)) {
        ...
        breakdown += `Base: R${price.toFixed(2)} x ${people} people x ${duration} hour(s) = R${base.toFixed(2)}<br>`
      } else {
        base = price * duration
        breakdown += `Base: R${price.toFixed(2)} x ${duration} = R${base.toFixed(2)}<br>`
      }
    }
    ...
    if (catering) {
      ...
      extras += catCost
      breakdown += `Catering: R${EXTRAS.Catering.toFixed(2)} x ${cateringPeople} = R${catCost.toFixed(2)}<br>`
    }
    if (internet) {
  ...
  document.getElementById("costBreakdown").innerHTML = breakdown || '<span style="color:#888">No selection</span>'
  document.getElementById("costResult").textContent = "R" + cost.toFixed(2)

  // Show/hide catering people input
  document.getElementById("cateringPeople").style.display = catering ? "" : "none"
...
document.getElementById("extraProjector").addEventListener("change", updateCostCalculator)
document.getElementById("extraCatering").addEventListener("change", updateCostCalculator)
document.getElementById("cateringPeople").addEventListener("input", updateCostCalculator)
document.getElementById("extraInternet").addEventListener("change", updateCostCalculator)
document.getElementById("people").addEventListener("input", updateCostCalculator)
...
function syncBookingFormToCalculator() {
  ...
  const dur = document.getElementById("event").getAttribute("data-duration") || 1
  if (cat) document.getElementById("category").value = cat
...
function showBookingSummary(
  ...
  if (category && facility && duration > 0) {
    ...
    summary.innerHTML = `<strong>Booking Summary:</strong><br>
            ...
            ${perPersonFacilities.includes(facility) ? `People: ${people}<br>` : ""}
